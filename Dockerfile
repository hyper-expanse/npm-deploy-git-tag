...
LABEL description="Docker image for executing `npm-deploy-git-tag`."
...
# We install the `ca-certificates` package to have access to a standard collection of CA root certificates for HTTPS operations that the `npm-deploy-git-tag` tool executes to interact with an npm-compatible registry.
# We install the `git` package for Git operations that the `npm-deploy-git-tag` tool executes.
RUN apk update && \
...
# Install only production dependencies for the `npm-deploy-git-tag` package.
# We create a symbolic link in the global binary directory that points to the `npm-deploy-git-tag` executable.
